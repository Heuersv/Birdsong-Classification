...
# Dataset params
DATASET_PATH = 'dataset/train/specs/spec_orig/'
...
MODEL_PATH = 'model/'
PRETRAINED_MODEL = 'birdCLEF_TUCMI_Run1_Model.pkl'  # 'pretrained_model.pkl'
LOAD_OUTPUT_LAYER = False
EPOCH_START = 1
RUN_NAME = 'Orig_Model1_pretrained'
...
def showConfusionMatrix(epoch):
    ...
    for i, j in itertools.product(range(min(CONFMATRIX_MAX_CLASSES, cmatrix.shape[0])),
                                  range(min(CONFMATRIX_MAX_CLASSES, cmatrix.shape[1]))):
...
def showProgress(stat, duration, current, end=batches_per_epoch, update_interval=5, simple_mode=False):
    ...
    if p > last_update and p % update_interval == 0 or last_update == -1:
        if simple_mode:
            ...
            print(stat.upper() + ': [' + progress + '] BATCHES ' + str(current) + '/' + str(end) + ' (' + str(
                p) + '%) - ' + str(r) + ' min REMAINING\r')
        last_update = p
