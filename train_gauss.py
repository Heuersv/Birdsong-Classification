...
#IM_DIM = 1 #Depending on STRATEGY (see below)
...
MODEL_PATH = 'model/'
PRETRAINED_MODEL = None #'birdCLEF_TUCMI_Run1_Model.pkl' #'pretrained_model.pkl'
...
def openImage(path, useCache=USE_CACHE, strategy=STRATEGY):
    ...
    if path in CACHE:
        ...
        if strategy == 'abs':
            ...
            img5 = zipfolder['specfreq_real']
            img6 = zipfolder['specfreq_imag']
            img1 = np.reshape(np.log10(np.abs(img1 + 1j*img2) + np.finfo(np.float32).tiny), [1, IM_SIZE[1], IM_SIZE[0]])
            img2 = np.reshape(np.log10(np.abs(img3 + 1j*img4) + np.finfo(np.float32).tiny), [1, IM_SIZE[1], IM_SIZE[0]])
            img3 = np.reshape(np.log10(np.abs(img5 + 1j*img6) + np.finfo(np.float32).tiny), [1, IM_SIZE[1], IM_SIZE[0]])
            img = np.concatenate([img1,img2,img3], axis=0)
            for dimension in range(3):
                img[dimension] -= np.min(img[dimension])
                img[dimension] /= np.max(img[dimension])
            ...
            raise ValueError("Please use a different strategy, currently supported: 'abs', 'db', 'raw', and the same with 'all' attached for differents resolutions in time and frequency.")
